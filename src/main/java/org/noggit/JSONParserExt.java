...
/**
 * Class complimentary to the JSONParser, but allowing one to reuse memory buffers, reinit paser, etc.
 */
public class JSONParserExt {
    ...
    char[] buf;  // input buffer with JSON text in it
    ...
    private boolean bool; // bo-olean value read
    private long lval;    // long value read
    private int nstate;   // current state while reading a number

    public JSONParserExt(char[] data, int start, int end, byte[] stack, CharArr localBuffer, CharArr tmp) {
        this.stack = stack;
        this.out = localBuffer;
        this.tmp = tmp;

        initBase(data, start, end);
    }
    ...
    private void initBase(char[] data, int start, int end) {
        this.buf = data;
        ...
        this.event = 0;
        this.stringTerm = 0;
        this.ptr = 0;     // pointer into the stack of parser states

        // idea - if someone passes us a CharArrayReader, we could
        // directly use that buffer as it's protected.
        this.state = 0;  // current parser state
        // info about value that was just read (or is in the middle of being read)
        this.valstate = 0;
        this.bool = false;
        this.lval = 0;
